...
telegram_bot_sendtext("Logged Niru")
...
while True:
    try:
        ...
        if limit_text != "https://expert.chegg.com/qna/authoring/answer":
           ...
           telegram_bot_sendques("LIMIT Niru")
           time.sleep(n)
        ...
        if text_to_copy == "Thank you for your efforts on Chegg Q&A! Unfortunately, no Qs are available in your queue at the moment.":
            ...
            if i <= 1:
                ...
                status = f"UP Running...  {i/10} Niru"
                telegram_bot_sendtext(status)
            i += 1
        else:
            telegram_bot_sendques("Niru")
            time.sleep(720)
    except Exception as e:
        telegram_bot_sendtext("An error occurred Niru")
# Quit the WebDriver
driver.quit()
